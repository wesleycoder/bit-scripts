...
export async function main(ns: NS) {
  ...
  if (help.includes(<string>ns.args[0])) {
    ns.tprintf(
  ...
  } else if (properties.includes(<string>ns.args[0])) {
    ns.tprintf(`INFO -\n* All Properties:`);
    ns.tprintf(
    ...
    ns.tprintf(
      `- corporation     | Corporation                    | Namespace for corporation functions.`
    );
    ns.tprintf(
    ...
    ns.tprintf(
      `- stanek          | Stanek                         | Namespace for stanek functions.`
    );
    ns.tprintf(
      `- stock           | TIX                            | Namespace for stock functions.`
    );
    ns.tprintf(
      `- ui              | UserInterface                  | Namespace for user interface functions.`
    );
  } else if (methods.includes(<string>ns.args[0])) {
    ns.tprintf(`INFO -\n* All Methods:`);
    ns.tprintf(
    ...
    ns.tprintf(
      `- print(msg)                                  | Prints a value or a variable to the script’s logs.`
    );
    ns.tprintf(
    ...
    ns.tprintf(
      `- scan(host)                                  | Get the list servers connected to a server.`
    );
    ns.tprintf(
    ...
    ns.tprintf(
      `- toast(msg, variant)                         | Queue a toast (bottom-right notification).`
    );
    ns.tprintf(
  ...
  } else if (bladeburner.includes(<string>ns.args[0])) {
  ...
  } else if (codingcontract.includes(<string>ns.args[0])) {
  ...
  } else if (corp.includes(<string>ns.args[0])) {
  ...
  } else if (formulas.includes(<string>ns.args[0])) {
    ...
    ns.tprintf(
      `- calculateExp(skill, skillMult)  | Calculate exp for skill level.`
    );
    ns.tprintf(`- calculateSkill(exp, skillMult)  | Calculate skill level.`);
  } else if (gang.includes(<string>ns.args[0])) {
  ...
  } else if (haccnet.includes(<string>ns.args[0])) {
    ...
    ns.tprintf(
      `- getStudyMult()                   | Get the multipler to study.`
    );
    ns.tprintf(
      `- getTrainingMult()                | Get the multipler to training.`
    );
    ns.tprintf(
  ...
  } else if (sleeve.includes(<string>ns.args[0])) {
    ...
    ns.tprintf(
      `- setToCommitCrime(sleeveNumber, name)                          | Set a sleeve to commit crime.`
    );
    ns.tprintf(
  ...
  } else if (stanek.includes(<string>ns.args[0])) {
  ...
  } else if (stocks.includes(<string>ns.args[0])) {
    ...
    ns.tprintf(
      `- getSaleGain(sym, shares, posType)           | Calculate profit of setting stocks.`
    );
    ns.tprintf(
  ...
  } else if (userinterface.includes(<string>ns.args[0])) {
    ns.tprintf(`INFO -\n* All Methods for User Interface:`);
    ns.tprintf(`- getTheme()  | Get the current theme`);
  } else if (ns2wait.includes(<string>ns.args[0])) {
    ns.tprintf(
