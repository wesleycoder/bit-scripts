 ...
 * @type {{syncOnWrite: boolean, jsonSpaces: number}}
 */
 ...
 */
export const validateJSON = function (fileContent) {
  try {
 ...
 * @param {boolean} [options.target] Target server hostname.
 ...
 * @constructor
 */
export function JSONdb(ns, filePath, options) {
  ...
  let stats;
  try {
    stats = this._ns.fileExists(filePath);
  } catch (err) {
    if (err.code === 'ENOENT') {
      /* File doesn't exist */
      return;
  ...
  /* File exists */
  try {
    this._ns.fileExists(filePath, options.target);
  } catch (err) {
    throw new Error(
 ...
 */
JSONdb.prototype.set = async function (key, value) {
 ...
 */
JSONdb.prototype.delete = async function (key) {
 ...
 */
JSONdb.prototype.sync = async function () {
  try {
    await this._ns.write(
      this.filePath,
